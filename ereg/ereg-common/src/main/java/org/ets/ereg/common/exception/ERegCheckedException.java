 ...
 *  -------
 */
public class ERegCheckedException extends Exception {
	...
	public ERegCheckedException() {
        super();
        date = new Date();
        refNumber = ExceptionUtil.buildRefNumber();
	}

	public ERegCheckedException(String message, Throwable anOriginalException) {
	...
	public ERegCheckedException(String message) {
		super(message);
        date = new Date();
        refNumber = "" + date.getTime();
	}

	public ERegCheckedException(Throwable anOriginalException) {
		super(anOriginalException);
        if (anOriginalException instanceof ERegCheckedException) {
        	ERegCheckedException aERegException = (ERegCheckedException) anOriginalException;
            date = aERegException.getDate ();
            refNumber = aERegException.getRefNumber ();
        } else if (anOriginalException instanceof ERegRuntimeException) {
        	ERegRuntimeException anERegRuntimeException = (ERegRuntimeException) anOriginalException;
            date = anERegRuntimeException.getDate ();
            refNumber = anERegRuntimeException.getRefNumber ();
    ...
    /**
     * Adds one key/value pair to the <b>data</b> instance variable. Both key and value must be a Strings.
     */
    ...
    public String toString() {
        ...
        for (Iterator i = data.keySet().iterator(); i.hasNext(); ) {
            ...
            sbMessage.append("\n\t").append(internalKey[0]).append(": ").append(data.get(internalKey)).append(" (added by " + internalKey[1]).append(")");
        }
