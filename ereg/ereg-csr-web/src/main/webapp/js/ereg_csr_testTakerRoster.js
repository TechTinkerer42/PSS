/* Start - Test Taker Roster Search */
...
$(document).ready(function() {
	...
	if(document.getElementById('action').value == 'backButton'){
		var form = $("#searchForm");
			$( "span.errorSpan" ).empty();
			var target = "searchFormResultsTable";
			$("#searchFormResultsTable").empty();
			ajaxSearch("searchForm", document.getElementById('pageNo').value,'');
			 $("#searchForm").find('.searchCriteriaPanel').slideToggle("slow");
	}
...
function ajaxSearch(idSearchForm, pageNo , action) {
	...
	$('#'+idSearchForm).find('input[name="rowperPage"]').val(document.getElementById('rowperPage').value);
	$('#'+idSearchForm).find('select[name="selRecordPerPage"]').val(document.getElementById('rowperPage').value);
...
function setRecordPerPage(){
	...
	//alert("Setting to " + document.getElementById('rowperPage').value);
...
function processAjaxLoadCenterData(data){
	...
	$('#testCenterId').append("<option value=''>Select</option>");
	for (col in data) {
...
function processAjaxLoadFormData(data){
	$('#testFormCode option').remove();	
	$('#testFormCode').append("<option value=''>Select</option>");
	for (col in data) {
