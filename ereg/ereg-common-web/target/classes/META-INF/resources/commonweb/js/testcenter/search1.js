	...
	$('button[id=searchButton]').click(function() {
		...
		geocoder.geocode({
		    ...
		    function(results, status) {
				...
				//	alert ($('#latitudeDegree').val());
				//	alert ($('#longitudeDegree').val());
...
function doSearch(){
	...
	/*
	Note:
	The next 2 lines of code and this note itself has to be removed when google lookup is in place
	With Google lookup in place, value of latitudeDegree and longitudeDegree will get set in event handlers of  city, state or zip
	*/
	//$('#latitudeDegree').val(40.6298702);
	//$('#longitudeDegree').val(-74.3033176);
...
function processSearchResult(target, data) {
	if(data.hasErrors){
		...
		var statusMessage = '<ul><li><span>' + '[Placeholder error message. No test centers found near the location you provided. Change search criteria and try again.]' + '</span></li></ul>';
